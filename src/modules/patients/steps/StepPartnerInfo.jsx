...
export default function StepPartnerInfo({ data, update }) {
  return (
    <div>
      ...
      <div className={styles.section}>
        ...
        <div className={styles.grid2}>
          <Input label="Partner Name" value={data.partnerName}
            onChange={e => update({ partnerName: e.target.value })} placeholder="Full name" />
          <Input label="Partner Age" type="number" value={data.partnerAge}
            onChange={e => update({ partnerAge: e.target.value })} placeholder="Age in years" suffix="yrs" />
        </div>
        <div style={{ marginTop: '1rem' }}>
          <Input label="Partner Occupation" value={data.partnerOccupation}
            onChange={e => update({ partnerOccupation: e.target.value })} placeholder="Optional" />
        </div>
        <div style={{ marginTop: '1rem' }}>
